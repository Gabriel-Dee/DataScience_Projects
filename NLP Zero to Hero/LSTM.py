...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.BiDirectional(tf.keras.layers.LSTM(64, return_sequences=True)), # here we have stacked the layers so that the output of one is fed into the next 
    tf.keras.layers.BiDirectional(tf.keras.layers.LSTM(32)), # in a scenario like this we have two layers so only the first has the return_sequences parameter, in case of 3 first 2 should have and so on
